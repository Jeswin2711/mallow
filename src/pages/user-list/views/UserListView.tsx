...
const UserListView = ({ view, search }: { view: string, search:string }) => {
  ...
  let filteredData = userList?.data?.filter((obj : UserDataType)  => obj.first_name.toLowerCase().includes(search.toLowerCase()) || obj.last_name.toLowerCase().includes(search.toLowerCase())) || []
  ...
  return (
    <div className="user-list-view-page">
      ...
      {showModal && (
        <UserAction
          userData={selectedUserData as UserDataType}
